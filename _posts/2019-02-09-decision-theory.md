...
- Given a set of samples from the joint distribution $$ p(x,y) $$ where $$ x \in \mathbb{R}^d $$ and $$ y \in \{0,1\} $$, learn a separating function that is able to correctly divide unseen samples from the same distribution. Note that $ x $ encodes the information provided by the records, and $y$ represents f a patient is sick or not.

In this case, when we receive new instance we are interested in
